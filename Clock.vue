...
export default {
  ...
  methods: {
    ...
    startWork : function(time){
      this.start = time;
      this.items.push({day: this.day,start: this.time,end:''})
    },
    endWork : function(time){
      this.end = time;
      this.items.push({day: this.day,start: '',end: this.time})
    },
  },
}
</script>
<template>
  <div>
    ...
    <button @click="startWork(hours + ':' + minutes + ':' + seconds)">出勤</button>
    <button @click="endWork(hours + ':' + minutes + ':' + seconds)">退勤</button>
    <ul>
      <li v-for="item in items" :key="item.date">
        {{item.starts}} / {{item.end}}
      </li>
    </ul>
    <p>{{start}}</p>
  </div>
